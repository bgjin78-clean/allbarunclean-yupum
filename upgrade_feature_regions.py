...
def related_links(slug):
    ...
    return "\n".join(
        f'<a href="/regions/{s}/">{ALL_REGION_NAMES.get(s, s)} 유품정리</a>'
        for s in items
    )

def schemas(region_type, name, slug):
    title = f"{name} 유품정리 · 고독사청소 · 특수청소 | 올바른수거"
    desc = f"올바른수거는 {name} 지역 유품정리, 고독사청소, 특수청소를 진행합니다. 유품 분류, 정리, 소독, 폐기물 반출까지 상담 가능합니다."
    ...
    data = [
        {
            ...
            "mainEntity": [
                {
                    "@type": "Question",
                    "name": f"{name} 유품정리 비용은 어떻게 정해지나요?",
                    "acceptedAnswer": {
                        "@type": "Answer",
                        "text": "유품의 양, 공간 크기, 폐기물 반출량, 엘리베이터 유무, 주차 환경, 특수청소 필요 여부에 따라 달라집니다."
                    }
                },
                {
                    "@type": "Question",
                ...
                {
                    "@type": "Question",
                    "name": f"{name} 유품정리와 특수청소를 함께 맡길 수 있나요?",
                    "acceptedAnswer": {
                        "@type": "Answer",
                        "text": "현장 상황에 따라 유품정리와 고독사청소, 특수청소를 함께 진행할 수 있습니다."
                    }
                }
            ]
        },
...
def generate():
    ...
    for region_type, name, slug in FEATURE_REGIONS:
        ...
        print(f"업그레이드 완료: {name} /regions/{slug}/")

    print(f"\n완료: 대표지역 {len(FEATURE_REGIONS)}개 페이지 업그레이드")
